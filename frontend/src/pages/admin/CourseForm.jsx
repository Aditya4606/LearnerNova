    ...
    <div className="h-full flex flex-col relative bg-[#F5F0EB] overflow-y-auto pb-32">
      ...
      <div className="sticky top-0 z-50 bg-[#F5F0EB]/90 backdrop-blur border-b border-[#EAE4DD] px-8 py-4 flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <button onClick={() => navigate(-1)} className="text-[#8A817C] hover:text-[#FB460D] interactive">
            <ArrowLeft size={20} />
          </button>
          <input 
            type="text" 
            defaultValue={id === 'new' ? 'Untitled Course' : 'Advanced React Patterns'}
            className="bg-transparent border-none text-[18px] font-bold text-[#141314] outline-none"
          />
        </div>
      ...
      <div className="max-w-4xl mx-auto w-full p-12">
        {/* Cover Image Upload */}
        <div className="border border-dashed border-[#EAE4DD] h-64 w-full mb-12 flex flex-col items-center justify-center cursor-pointer hover:border-[#FB460D]/50 transition-colors interactive group bg-[#FFFFFF]">
          <div className="w-16 h-16 rounded-full bg-[#EAE4DD] flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
            <Plus size={24} className="text-[#141314]" />
          </div>
          <p className="text-[10px] uppercase tracking-widest text-[#8A817C] font-bold">DRAG & DROP COURSE COVER</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8 mb-16">
          ...
          <div className="flex flex-col mb-4">
            ...
            <select className="bg-transparent border-0 border-b border-[#EAE4DD] text-[#141314] text-[15px] px-0 py-3 w-full outline-none interactive appearance-none cursor-pointer">
              <option className="bg-[#FFFFFF]">John Instructor</option>
              <option className="bg-[#FFFFFF]">Admin Team</option>
            </select>
          </div>
        </div>
        ...
        {activeTab === 'CONTENT' && (
          <div className="space-y-0">
            {['Compound Components', 'Render Props', 'Custom Contexts'].map((lesson, i) => (
              <div key={i} className="flex items-center space-x-4 py-4 border-b border-[#EAE4DD] group hover:bg-[#FFFFFF] px-4 -mx-4 transition-colors">
                <GripVertical size={16} className="text-[#EAE4DD] cursor-move" />
                <div className="flex-1">
                  <p className="text-[14px] text-[#141314] font-semibold">{lesson}</p>
                </div>
        ...
        {activeTab === 'OPTIONS' && (
          <div className="space-y-4">
            {['Require sequential learning', 'Drip content weekly', 'Publicly visible'].map((opt, i) => (
              <div key={i} className="p-6 border border-[#EAE4DD] bg-[#FFFFFF] flex items-center justify-between group hover:border-l-[#FB460D] hover:border-l-[4px] transition-all interactive cursor-pointer">
                <div>
                  <p className="text-[14px] font-semibold text-[#141314] mb-1">{opt}</p>
                  <p className="text-[11px] text-[#8A817C]">Enable this setting to apply specific routing.</p>
                </div>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-[#EAE4DD]">
        <div className="flex space-x-8">
          {['CONTENT', 'DESCRIPTION', 'ATTACHMENTS'].map((t, i) => (
            <button key={t} className={`text-[10px] uppercase font-bold tracking-widest interactive ${i === 0 ? 'text-[#FB460D]' : 'text-[#8A817C] hover:text-[#141314]'}`}>
              {t}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-4">
            {['VIDEO', 'TEXT', 'PDF', 'AICC'].map((type, i) => (
              <div key={type} className={`p-4 border text-center interactive cursor-pointer font-bold tracking-widest text-[10px] uppercase ${i === 0 ? 'border-[#FB460D] text-[#FB460D] bg-[#FB460D]/5' : 'border-[#EAE4DD] text-[#8A817C] hover:border-[#8A817C]'}`}>
                {type}
              </div>
            ))}
          </div>
        ...
        <div className="flex items-center justify-between p-4 border border-[#EAE4DD] bg-[#F5F0EB]">
          <span className="text-[11px] font-bold text-[#141314] tracking-widest uppercase">ALLOW DOWNLOAD</span>
          <Toggle checked={download} onChange={setDownload} />
        </div>
      </div>
      
      <div className="flex justify-end space-x-4 mt-12 pt-6 border-t border-[#EAE4DD]">
